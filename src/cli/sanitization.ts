...
export const isSafeVariableIdentifier = (value: string) =>
  /^[a-z$_][a-z0-9_$]*$/i.test(value)

export const makeSafeVariableIdentifier = (value: string) =>
  isSafeVariableIdentifier(value)
    ? value
    : camelCase(replaceInvalidWithUnderscore(value)) || 'arg'

const escapeQuotes = (value: string) =>
  value.replace(/['"]/g, val => `\\${val}`)

export const makeSafePropertyIdentifier = (value: string) =>
  isSafeVariableIdentifier(value) ? value : `'${escapeQuotes(value)}'`
