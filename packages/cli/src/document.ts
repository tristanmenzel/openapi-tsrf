...
import { generateSchema, hasComponentRef, resolveParameterReference } from './schemas'
import { generateOperation } from './operations'
import { ParsingError } from './ParsingError'

export function* generateDocumentParts(document: Swagger.Spec3, includeHeaders: string[] = []): AsyncDocumentParts {
  const hasOperations =
    iterateDictionary(document.paths).flatMap(([_, pathObj]) => methods(pathObj).map(([_, operation]) => operation)).length > 0
  if (hasOperations) {
  ...
  for (const [name, schemaObj] of iterateDictionary(document.components.schemas)) {
  ...
  if (hasOperations) {
    ...
    for (const [pathStr, pathObj] of iterateDictionary(document.paths)) {
      for (const [method, operation] of methods(pathObj)) {
        try {
          const pathParams = pathObj.parameters?.map((p) => (hasComponentRef(p) ? resolveParameterReference(document, p.$ref) : p))
          yield* generateOperation(document, pathStr, method, includeHeaders, {
            ...operation,
            parameters: [
              ...(operation.parameters?.map((p) => (hasComponentRef(p) ? resolveParameterReference(document, p.$ref) : p)) ?? []),
              ...(pathParams ?? []),
            ],
          })
        } catch (error) {
          throw new ParsingError(`Failed to generate operation for path ${method}:${pathStr}`, error)
        }
      }
    }
    yield DecIndent
