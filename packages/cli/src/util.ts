...
export function typeCheckerFor<TTarget>() {
  return {
    hasProp<TSource extends object, TProp extends keyof TTarget>(
      source: TSource,
      prop: TProp,
    ): source is TSource & {
      ...
      return Object.hasOwn(source, prop) && (source as any)[prop] !== undefined
    },
  }
}

export const notFalsy = <T>(value: T): value is Exclude<T, 0 | '' | false | null | undefined> => {
