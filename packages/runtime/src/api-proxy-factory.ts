...
export type ProxyForWithRequiredConfig<TFac, TConfig> = ProxyForWithRequiredConfigInternal<TFac, keyof TFac, TConfig>
export type ProxyForWithRequiredConfigInternal<TFac, TKey extends keyof TFac, TConfig> = {
  [key in TKey]: TFac[key] extends (...args: infer TArgs) => AnyRequest<infer TResponse>
    ? (arg0: TConfig, ...args: TArgs) => Promise<TResponse>
    : TFac[key]
}

export type MakeRequest<TConfig> = <TResponse>(request: AnyRequest<TResponse>, config: TConfig | undefined) => Promise<TResponse>

export type NotDefaultedConfig<TConfig, TDefaultedConfig extends Partial<TConfig>> = Partial<TDefaultedConfig> &
...
export class ApiProxyFactory<TConfig = never> {
  ...
  createProxy<TRequestFactory extends object, TConfigDefaults extends Partial<TConfig>>(
  ...
  createProxyWithRequiredConfig<TRequestFactory extends object, TConfigDefaults extends Partial<TConfig>>(
    requestFactory: TRequestFactory,
    defaultConfig?: TConfig,
  ): ProxyForWithRequiredConfig<TRequestFactory, NotDefaultedConfig<TConfig, TConfigDefaults>> {
  ...
  private createProxyInternal<TRequestFactory extends object>(
    ...
    return new Proxy<TRequestFactory>(requestFactory, {
      get(target: TRequestFactory, property: string | symbol, receiver: any): any {
        if (typeof property === 'string' && typeof target[property as keyof TRequestFactory] === 'function' && property !== 'prototype') {
          if (proxyCache[property]) return proxyCache[property]

          async function makeRequestIntl(config: TConfig, args: any[]) {
            const requestFactory = Reflect.get(target, property, receiver) as (...args: any[]) => AnyRequest<any>

            const request = requestFactory(...args)

            try {
              return factory.makeRequest(request, defaultConfig ? { ...defaultConfig, ...config } : config)
            } catch (error: unknown) {
