...
export abstract class RequestFactory {
  static getThings(a: number, b: string, c: boolean): GetRequest<GetRequest<unknown>> {
    return {
      method: 'GET',
      url: `/path/${a}/${b}/${c}`,
    }
...
describe('ApiProxyFactory', () => {
  describe('With no config', () => {
    const myMockRequester = vi.fn((request: AnyRequest<any>): any => Promise.resolve(request))
  ...
  describe('With optional config', () => {
    ...
    const myMockRequester = vi.fn((request: AnyRequest<any>, config: MyConfig | undefined): any => Promise.resolve(request))
    ...
    it('Passes config to makeRequest when it is present', async () => {
      await myApi.getThings.withConfig({ value: 'abc123' }).execute(1, 'b', false)
